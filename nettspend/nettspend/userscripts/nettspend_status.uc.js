...

{
	...
	var { PrefCalls, LocaleUtils, waitForElement, setAttributes } = ChromeUtils.importESModule("chrome://modules/content/NSUtils.sys.mjs");
	...
	var NettspendStatusBarManager = {
			...
			return `
				<vbox id="browser-bottombox">
					<statusbar id="status-bar">
						...
						<statusbarpanel id="offline-status" class="statusbarpanel-iconic" label="${LocaleUtils.str(statusBundle, "statusbar_panel_offlinestatus.label")}">
							<image class="statusbarpanel-icon" />
						</statusbarpanel>			
						<statusbarpanel id="security-button" class="statusbarpanel-iconic" tooltiptext="${LocaleUtils.str(statusBundle, "statusbar_panel_securitybutton.tooltiptext")}">
							<image class="statusbarpanel-icon" />
						</statusbarpanel>
		...
		init() {
			...
			document.getElementById("offline-status").addEventListener("click", this.handleEvent);
			document.getElementById("security-button").addEventListener("click", this.handleEvent);
		...
		_onPopupShowing() {
			...
			if (item)
			{
				item.forEach(elem => {
					...
					let pref = Services.prefs.getBoolPref("nettspend.status-bar.enabled", false);
		...
		_applyStatusBarEnabledPrefs() {
			let newState = Services.prefs.getBoolPref("nettspend.status-bar.enabled", false);
	...
	waitForElement("#menu_viewPopup").then((menu) => {
		let statusBarItem = document.createXULElement("menuitem");
		statusBarItem.id = "menu_NettspendStatusBar";
		statusBarItem.addEventListener("command", () => {
			NettspendStatusBarManager.setStatusBarState(!Services.prefs.getBoolPref("nettspend.status-bar.enabled", false));
		});
